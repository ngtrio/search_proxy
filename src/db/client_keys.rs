...
impl Database {
    ...
    pub async fn create_client_key(
        ...
        let result = sqlx::query(
            "INSERT INTO client_api_keys(name, prefix, digest)
             VALUES (?, ?, ?)",
        )
        .bind(name)
        .bind(prefix)
    ...
    pub async fn revoke_client_key(&self, id: i64) -> Result<(), sqlx::Error> {
        sqlx::query("UPDATE client_api_keys SET status = 'revoked' WHERE id = ?")
            .bind(id)
            .execute(&self.pool)
            .await
            .map(|_| ())

...
type Page = "overview" | "providers" | "keys" | "requests" | "settings";
type ProviderForm = Pick<Provider, "kind" | "name" | "endpoint" | "weight" | "enabled" | "timeout_seconds"> & { token: string };
const emptyProvider: ProviderForm = { kind: "searchix", name: "", endpoint: "https://", token: "", weight: 1, enabled: true, timeout_seconds: 120 };
...
function Login({ done }: { done: () => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    catch { setError("Invalid credentials or temporarily throttled"); }
  }
...
function Panel({ signedOut }: { signedOut: () => void }) {
  ...
  return <div className="shell"><aside><div><p className="eyebrow">TAVILY</p><h2>Gateway</h2></div><nav>{(["overview", "providers", "keys", "requests", "settings"] as Page[]).map(value => <button className={value === page ? "active" : ""} onClick={() => setPage(value)} key={value}>{value}</button>)}</nav><button className="signout" onClick={logout}>Sign out</button><small>Metadata only · no query retention</small></aside><main><header><p className="eyebrow">OPERATIONS</p><h1>{page[0].toUpperCase() + page.slice(1)}</h1></header>{page === "overview" ? <Overview /> : page === "providers" ? <Providers /> : page === "keys" ? <Keys /> : page === "requests" ? <Requests /> : <Settings />}</main></div>;
}
...
function Providers() {
  ...
  function edit(provider?: Provider) { setEditing(provider?.id ?? 0); setForm(provider ? { kind: provider.kind, name: provider.name, endpoint: provider.endpoint, token: "", weight: provider.weight, enabled: provider.enabled, timeout_seconds: provider.timeout_seconds } : emptyProvider); }
...
function Settings() {
  const [form, setForm] = useState({ default_timeout_seconds: 120 }); useEffect(() => { void api<typeof form>("/settings").then(setForm); }, []);
  async function save(event: FormEvent) { event.preventDefault(); await api("/settings", { method: "PUT", body: JSON.stringify(form) }); }
  return <form className="settings editor" onSubmit={save}><h3>Operational defaults</h3><label>Default timeout seconds<input type="number" min="1" value={form.default_timeout_seconds} onChange={e => setForm({ ...form, default_timeout_seconds: Number(e.target.value) })} /></label><button>Save settings</button><h3>Privacy boundary</h3><p>Queries, arguments, results, credentials, and password hashes are excluded from observability views.</p></form>;
}
